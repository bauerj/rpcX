...
def rpc_process_message(rpc, message):
    ...
    if rpc == JSONRPCv2:
        ...
        if 'result' in message and 'error' not in message:
            message['error'] = None
    return rpc.process_message(json.dumps(message).encode())
    ...
    if rpc == JSONRPCv2:
        for item in batch:
            if isinstance(item, dict):
                item['jsonrpc'] = '2.0'
    return rpc.process_message(json.dumps(batch).encode())
...
def test_base():
    with pytest.raises(NotImplementedError):
        JSONRPC._message_id(None, True)
    JSONRPC()._validate_message(None)
    with pytest.raises(NotImplementedError):
        JSONRPC._request_args(None)
...
def test_bad_encoding():
    message = b'123\xff'
    for rpc in rpcs:
        item = rpc.process_message(message)
...
def test_request_bad_method():
    for rpc in rpcs:
        ...
        item = rpc_process_message(rpc, message)
...
def test_JSONRPCLoose_responses():
    ...
    item = rpc.process_message(json.dumps(message).encode())
...
def test_batch_not_allowed():
    for rpc in no_batch_rpcs:
        item = rpc.process_message(b'[]')
        assert_is_error(item, 'dict', rpc.INVALID_REQUEST, None)
        with pytest.raises(RuntimeError):
            rpc.batch_bytes(None)
...
def test_batch_bytes_from_parts():
    for rpc in rpcs:
        assert rpc.batch_bytes_from_parts([]) == b''
        assert rpc.batch_bytes_from_parts([b'1']) == b'[1]'
        assert rpc.batch_bytes_from_parts([b'1', b'2']) == b'[1, 2]'
        # An empty part is not valid, but anyway.
        assert rpc.batch_bytes_from_parts([b'1', b'', b'[3]']) == b'[1, , [3]]'


def test_request_bytes():
    requests = [
        RPCRequest('sub', [], 3),
        RPCRequest('meth', [2, 3], None),
        RPCRequest('meth', [2, 3], "id")
    ]
    for rpc in rpcs:
        for request in requests:
            assert rpc.request_bytes(request) == rpc.item_bytes(request)


def test_response_bytes():
    responses = [
        RPCResponse('result', 2),
        RPCResponse([2, 5], 2),
        RPCResponse(RPCError(3, "j"), 1)
    ]
    for rpc in rpcs:
        for response in responses:
            assert rpc.response_bytes(response) == rpc.item_bytes(response)


def test_error_bytes():
    error = RPCError(28, 'an error occurred', 3)
    for rpc in rpcs:
        assert rpc.error_bytes(error) == rpc.item_bytes(error)
...
def test_encode_payload():
    for rpc in rpcs:
        ...
        error = rpc.error_bytes(rpc.internal_error(None))
        assert rpc.encode_payload(b'foo') == error
        error = rpc.error_bytes(rpc.internal_error(4))
...
def test_JSONRPCv1_messages():
    ...
    with pytest.raises(TypeError):
        rpc.item_bytes(RPCRequest('foo', {}, 2))
    with pytest.raises(TypeError):
        rpc.item_bytes(RPCRequest('foo', {"x": 1}, 2))
...
def test_bad_implemntation():
    ...
    rpc = Mine
    item = rpc_process_message(rpc, {"method": "foo", "id": 5})
    assert_is_error(item, 'internal', rpc.INTERNAL_ERROR, 5)
    item = rpc_process_message(rpc, {"result": 6, "id": 5})
...
def test_protocol_detection():
    ...
    batch_bytes = JSONRPC.batch_bytes_from_parts
    for length in range(1, len(test_by_answer)):
        for combo in combinations(test_by_answer, length):
            batch = batch_bytes(test_by_answer[answer] for answer in combo)
